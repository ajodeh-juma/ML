# clear the workspace & set the working directory

rm(list=ls())
setwd('~/Desktop/ML/R/problems/predict_income/data/')

# Load libraries and set random number generator seed for reproducibility
...
test <- dt[sample(nrow(dt), 121)] # 25% of the data as test set
train <- dt[!test, on="ID"]       # use the rest of the data as training set (ha)

#------------------- Build an additional simple version of the test set in which every city in test is also seen in train -----#
